...
ifCleanToken <- function(cur_token){
  ...
  rule_list <- c("str_count(cur_token, pattern = '[A-Za-z0-9]') <= 0.5*nchar(cur_token)", # If the number of punctuation characters in a string is greater than the number of alphanumeric characters, it is garbage
                 ...
                 "gsub('([A-Za-z0-9])\\1\\1+', '', cur_token)!=cur_token", #If there are three or more identical characters in a row in a string, it is garbage
                 "(length(strsplit(gsub('[:a-z:]','',cur_token),NULL)[[1]]) > length(strsplit(gsub('[:A-Z:]','',cur_token),NULL)[[1]])) & (length(strsplit(gsub('[:a-z:]','',cur_token),NULL)[[1]]) < length(strsplit(cur_token,NULL)[[1]]))", #If the number of uppercase characters in a string is greater than the number of lowercase characters, and if the number of uppercase characters is less than the total number of characters in the string, it is garbage
...
# First 3 rules, 29 false
# Add 4th rule, 265 false
# Add 5th rule, 266 false
# Add 6th rule, 372 false
# Add 7th rule, 395 false
# Add 8th rule, 396 false
